        ...
        for i, key in enumerate(channel_texts):
            if int(key) > int(message_id) and int(message_ages[i]) <= 60 and len(channel_texts[key]) <= int(max_message_length):
                ...
                if bool(output) is False:
                    ...
                    if 'хвилин' not in channel_texts[key]:
                    ...
                    if result == 0:
                        ...
                        if critical == "true":
                            logging.warning(f"{channel}:{key} - {message_ages[i]} - {channel_texts[key]}")
                        else:
                            logging.info(f"{channel}:{key} - {message_ages[i]} - {channel_texts[key]}")
                        counter += 1
                        time.sleep(5)
