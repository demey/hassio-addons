...
def compile_regex_patterns(config):
    ...
    tts_replacements = [
        ...
        (re.compile(r"Чорнобильська ЗВ", re.I), "Чорнобильська зона"),
        # 4. Ракети та авіація (враховано латиницю/кирилицю x/х)
        ...
        (re.compile(r"1\s?шт(?:ук[иа]?)?\b\.?\s?", re.I), "одна штука "),
        (re.compile(r"2\s?шт(?:ук[иа]?)?\b\.?\s?", re.I), "дві штуки "),
        (re.compile(r"3\s?шт(?:ук[иа]?)?\b\.?\s?", re.I), "три штуки "),
        (re.compile(r"4\s?шт(?:ук[иа]?)?\b\.?\s?", re.I), "чотири штуки "),
        (re.compile(r"(\d+)\s?шт\b\.?\s?", re.I), r"'\1' штук "),
        (re.compile(r"(\d+(?:[.,]\d+)?)[xх]\b", re.I), r"\1"),
        # 7. Діапазони чисел
